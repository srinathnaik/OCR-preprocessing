...
/*Largest contour detection*/
void draw_contours1(Mat * src, Mat orig)
{
	Mat dst((*src).rows,(*src).cols,CV_8UC1,Scalar::all(0));
	vector<vector<Point> > contours;
	vector<Vec4i> hierarchy;
	...
	// return dst;
	/// Find contours
	findContours( *src, contours, hierarchy, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, Point(0, 0) );//originally used CV_RETR_TREE,CV_RETR_CCOMP
	...
	Mat csrc;
	cvtColor(*src, csrc, cv::COLOR_GRAY2RGB);
	// for all contours
	// for( int i = 0; i< contours.size(); i++ )
	// {
	...
	// for filtering contours for text
	...
	for( int idx = 0; idx< contours.size(); idx++ ){
        ...
        if (rect.height > 8 && rect.width > 8)
        {
	        if (r > 0.35 )
	        {
	            // rectangle(drawing, rect, Scalar(0, 255, 0), 2);
	            rectangle(csrc, rect, Scalar(0, 255, 0), 2);
	        }
	        else
	        	// rectangle(drawing, rect, Scalar(255, 0, 0), 2);
	        	rectangle(csrc, rect, Scalar(255, 0, 0), 2);
	    }
	    else 
	        // rectangle(drawing, rect, Scalar(0, 0, 255), 2);
	        rectangle(csrc, rect, Scalar(0, 0, 255), 2);

    }

	/// Show in a window
	namedWindow( "Contours", WINDOW_NORMAL );
	imshow( "Contours", csrc );
}
...
int main(int argc, const char** argv)
{
	...
	/*binarise*/
		Mat * s1 = new Mat();
		binarise(&img,s1);
		show(s1,"I1 : binarised");

	/*transform*/
		Mat * s2 = new Mat();
		morphological_transform(s1,s2,4);
		show(s2,"I2 : transformed");
	...
	waitKey(0);
	destroyWindow("I1 : binarised");
	destroyWindow("I2 : transformed");
	destroyWindow("I3 : contours");
	destroyWindow("rotated image");
	show(&img1,"rotated image");

	/*draw contours for the rotated image*/
		s3 = draw_contours(&img1);
		show(&s3,"II1 : contours");
